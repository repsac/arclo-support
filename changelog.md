# Changelog

Release notes for every published version of Arclo.

## 1.0

Initial public release.

**Gameplay**
- One-tap swing arcade: tap to swap the anchor orb and fling the pair forward through a run of gates.
- Two distinct physics modes, tracked separately: Orbit (constant-spin swing) and Pendulum (gravity swing).
- A chasing void that pushes back when you clear a gate and surges when you miss one.
- Ten difficulty tiers. Tiers 1 to 3 are free; tiers 4 to 10 unlock with a single one-time purchase.
- A learn-one-at-a-time mechanic ladder: big turns, moving gates, drifting hazards, spin-flips and tether zones, barriers, fly-ins, and pulsing gates, combining in the top tiers.
- Endless levels: each cleared level generates a slightly harder next one.
- Rare floating power-ups: Wide Gates (gates widen briefly) and Shield (absorbs one non-void hit briefly).
- A run timer and a saved best time for each level.

**Orientation and controls**
- Orbit plays horizontally or vertically, or follows your device automatically.
- Full hardware-keyboard support: Space to swap, Enter to continue, Esc to pause or go back, number keys to pick a tier.

**Audio**
- Original instrumental synthwave soundtrack that plays through the songs with crossfades, with separate gameplay and menu pools.
- Independent on/off and volume for sound effects and music, plus an option to start a fresh song each run or keep the soundtrack continuous.
- A built-in Soundtrack Player (with the full game) that behaves like a music app: device-volume playback, lock-screen and Control Center controls, background playback, and a scrub bar.

**Progress and social**
- Game Center leaderboards split by mode and by free or full band, recurring weekly.
- Optional iCloud sync for level progress, best scores, best times, and your unlock.
- A guided first-launch tutorial you can replay any time.

**Privacy**
- No accounts, no tracking, no analytics, no ads. Progress stays on your device, with optional iCloud sync handled entirely by Apple.
